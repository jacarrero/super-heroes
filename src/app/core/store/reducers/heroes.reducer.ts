...
export const heroesReducer = createReducer(
  ...
  on(HeroesActions.filterHeros, (state, { filterForm }) =>({
    ...state,
    heroes: Object.values(filterForm).every(value => value === null) ? state.originalDataHero : state.originalDataHero.filter((hero) => {
      return (
        filterForm?.name && hero.name.toLocaleLowerCase().includes((filterForm.name).toLowerCase() as string) ||
        filterForm?.age && hero.age === filterForm.age ||
        filterForm?.city && hero.city.toLowerCase().includes((filterForm.city as string).toLowerCase()) ||
  ...
  on(HeroesActions.editHeroSuccess, (state, { hero }) => ({
    ...
    originalDataHero: state.heroes.map(h => h.id === hero.id ? hero : h),
  }))
);