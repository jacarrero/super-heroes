        ...
        this.actions$.pipe(
            ...
            switchMap(() =>
                this.heroesService.getHeroes().pipe(
                    delay(2500),
                    map((heroes) =>
                        HeroesActions.loadHeroesSuccess({ heroes })
                    ),
                    catchError((error) =>
                        of(HeroesActions.loadHeroesFailure({ error }))
                    )
                )
            )
        )
        ...
        this.actions$.pipe(
            ...
            mergeMap(({ hero }) =>
                this.heroesService.createHero(hero).pipe(
                    ...
                    catchError((error) =>
                        of(HeroesActions.createHeroFailure({ error }))
                    )
                )
            ),
            tap(() => {
        ...
        this.actions$.pipe(
            ...
            mergeMap(({ hero }) =>
                this.heroesService.editHero(hero.id as string, hero).pipe(
                    ...
                    catchError((error) =>
                        of(HeroesActions.editHeroFailure({ error }))
                    )
                )
            ),
            tap(() => {
